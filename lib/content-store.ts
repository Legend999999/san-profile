...
export async function createContentProject(input: ProjectInput) {
  ...
  await saveGitHubFile(data, sha, `Add project: ${project.title}`);
  return project;
}

export async function updateContentProject(id: string, input: ProjectInput) {
  const { data, sha } = await getGitHubFile();
  ...
  await saveGitHubFile(data, sha, `Update project: ${updated.title}`);
  return updated;
}

export async function deleteContentProject(id: string) {
  const { data, sha } = await getGitHubFile();
  const project = data.projects.find((item) => item.id === id);
  data.projects = data.projects.filter((item) => item.id !== id);
  await saveGitHubFile(data, sha, `Delete project: ${project?.title ?? id}`);
}

export async function updateContentSettings(input: Partial<WebsiteSettings>) {
  const { data, sha } = await getGitHubFile();
  data.settings = {
    ...data.settings,
  ...
  await saveGitHubFile(data, sha, "Update website settings");
  return data.settings;
}
